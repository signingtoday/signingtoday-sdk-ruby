...
module SigningTodayAPIClient
  class UpdateOrganization
    ...
    attr_accessor :signature_appearance
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SigningTodayAPIClient::UpdateOrganization` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SigningTodayAPIClient::UpdateOrganization`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'default_rao')
        self.default_rao = attributes[:'default_rao']
      end

      if attributes.key?(:'dst_default_months')
        self.dst_default_months = attributes[:'dst_default_months']
      else
        self.dst_default_months = 3
      end
      ...
      if attributes.key?(:'signature_appearance')
        self.signature_appearance = attributes[:'signature_appearance']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          default_rao == o.default_rao &&
          dst_default_months == o.dst_default_months &&
          logo == o.logo &&
          name == o.name &&
          signature_appearance == o.signature_appearance
    end
    ...
    def hash
      [default_rao, dst_default_months, logo, name, signature_appearance].hash
    end
