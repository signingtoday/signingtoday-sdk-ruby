=begin
#Signing Today API

#

The version of the OpenAPI document: 1.5.0
Contact: smartcloud@bit4id.com
Generated by: https://openapi-generator.tech
OpenAPI Generator version: 4.1.0

=end

require 'date'

module SigningTodayAPIClient
  class SignatureImplementationResponse
    attr_accessor :certificate

    attr_accessor :channel

    attr_accessor :document

    attr_accessor :dst

    # Number of the groups which the signer belongs during digital signature transaction creation
    attr_accessor :group

    attr_accessor :identity

    attr_accessor :pades_subfilter

    attr_accessor :page

    attr_accessor :position

    attr_accessor :profile

    attr_accessor :provider_id

    attr_accessor :reason

    attr_accessor :signature

    attr_accessor :signature_appearance_uri

    attr_accessor :signature_text

    attr_accessor :signing_time

    attr_accessor :title

    class EnumAttributeValidator
      attr_reader :datatype
      attr_reader :allowable_values

      def initialize(datatype, allowable_values)
        @allowable_values = allowable_values.map do |value|
          case datatype.to_s
          when /Integer/i
            value.to_i
          when /Float/i
            value.to_f
          else
            value
          end
        end
      end

      def valid?(value)
        !value || allowable_values.include?(value)
      end
    end

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'certificate' => :'certificate',
        :'channel' => :'channel',
        :'document' => :'document',
        :'dst' => :'dst',
        :'group' => :'group',
        :'identity' => :'identity',
        :'pades_subfilter' => :'pades_subfilter',
        :'page' => :'page',
        :'position' => :'position',
        :'profile' => :'profile',
        :'provider_id' => :'provider_id',
        :'reason' => :'reason',
        :'signature' => :'signature',
        :'signature_appearance_uri' => :'signature_appearance_uri',
        :'signature_text' => :'signature_text',
        :'signing_time' => :'signing_time',
        :'title' => :'title'
      }
    end

    # Attribute type mapping.
    def self.openapi_types
      {
        :'certificate' => :'Certificate',
        :'channel' => :'SignatureImplementationResponseChannel',
        :'document' => :'String',
        :'dst' => :'Id',
        :'group' => :'Integer',
        :'identity' => :'Id',
        :'pades_subfilter' => :'String',
        :'page' => :'Integer',
        :'position' => :'Position',
        :'profile' => :'Profile',
        :'provider_id' => :'ProviderId',
        :'reason' => :'Reason',
        :'signature' => :'Id',
        :'signature_appearance_uri' => :'SignatureAppearanceUri',
        :'signature_text' => :'String',
        :'signing_time' => :'String',
        :'title' => :'Title'
      }
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SigningTodayAPIClient::SignatureImplementationResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SigningTodayAPIClient::SignatureImplementationResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'certificate')
        self.certificate = attributes[:'certificate']
      end

      if attributes.key?(:'channel')
        self.channel = attributes[:'channel']
      end

      if attributes.key?(:'document')
        self.document = attributes[:'document']
      end

      if attributes.key?(:'dst')
        self.dst = attributes[:'dst']
      end

      if attributes.key?(:'group')
        self.group = attributes[:'group']
      end

      if attributes.key?(:'identity')
        self.identity = attributes[:'identity']
      end

      if attributes.key?(:'pades_subfilter')
        self.pades_subfilter = attributes[:'pades_subfilter']
      end

      if attributes.key?(:'page')
        self.page = attributes[:'page']
      end

      if attributes.key?(:'position')
        self.position = attributes[:'position']
      end

      if attributes.key?(:'profile')
        self.profile = attributes[:'profile']
      end

      if attributes.key?(:'provider_id')
        self.provider_id = attributes[:'provider_id']
      end

      if attributes.key?(:'reason')
        self.reason = attributes[:'reason']
      end

      if attributes.key?(:'signature')
        self.signature = attributes[:'signature']
      end

      if attributes.key?(:'signature_appearance_uri')
        self.signature_appearance_uri = attributes[:'signature_appearance_uri']
      end

      if attributes.key?(:'signature_text')
        self.signature_text = attributes[:'signature_text']
      end

      if attributes.key?(:'signing_time')
        self.signing_time = attributes[:'signing_time']
      end

      if attributes.key?(:'title')
        self.title = attributes[:'title']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array.new
      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      pades_subfilter_validator = EnumAttributeValidator.new('String', ["ETSI.CAdES.detached"])
      return false unless pades_subfilter_validator.valid?(@pades_subfilter)
      signing_time_validator = EnumAttributeValidator.new('String', ["pdf"])
      return false unless signing_time_validator.valid?(@signing_time)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] pades_subfilter Object to be assigned
    def pades_subfilter=(pades_subfilter)
      validator = EnumAttributeValidator.new('String', ["ETSI.CAdES.detached"])
      unless validator.valid?(pades_subfilter)
        fail ArgumentError, "invalid value for \"pades_subfilter\", must be one of #{validator.allowable_values}."
      end
      @pades_subfilter = pades_subfilter
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] signing_time Object to be assigned
    def signing_time=(signing_time)
      validator = EnumAttributeValidator.new('String', ["pdf"])
      unless validator.valid?(signing_time)
        fail ArgumentError, "invalid value for \"signing_time\", must be one of #{validator.allowable_values}."
      end
      @signing_time = signing_time
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          certificate == o.certificate &&
          channel == o.channel &&
          document == o.document &&
          dst == o.dst &&
          group == o.group &&
          identity == o.identity &&
          pades_subfilter == o.pades_subfilter &&
          page == o.page &&
          position == o.position &&
          profile == o.profile &&
          provider_id == o.provider_id &&
          reason == o.reason &&
          signature == o.signature &&
          signature_appearance_uri == o.signature_appearance_uri &&
          signature_text == o.signature_text &&
          signing_time == o.signing_time &&
          title == o.title
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Integer] Hash code
    def hash
      [certificate, channel, document, dst, group, identity, pades_subfilter, page, position, profile, provider_id, reason, signature, signature_appearance_uri, signature_text, signing_time, title].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def self.build_from_hash(attributes)
      new.build_from_hash(attributes)
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      self.class.openapi_types.each_pair do |key, type|
        if type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[self.class.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[self.class.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[self.class.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[self.class.attribute_map[key]]))
        end # or else data not found in attributes(hash), not an issue as the data can be optional
      end

      self
    end

    # Deserializes the data based on type
    # @param string type Data type
    # @param string value Value to be deserialized
    # @return [Object] Deserialized data
    def _deserialize(type, value)
      case type.to_sym
      when :DateTime
        DateTime.parse(value)
      when :Date
        Date.parse(value)
      when :String
        value.to_s
      when :Integer
        value.to_i
      when :Float
        value.to_f
      when :Boolean
        if value.to_s =~ /\A(true|t|yes|y|1)\z/i
          true
        else
          false
        end
      when :Object
        # generic object (usually a Hash), return directly
        value
      when /\AArray<(?<inner_type>.+)>\z/
        inner_type = Regexp.last_match[:inner_type]
        value.map { |v| _deserialize(inner_type, v) }
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        k_type = Regexp.last_match[:k_type]
        v_type = Regexp.last_match[:v_type]
        {}.tap do |hash|
          value.each do |k, v|
            hash[_deserialize(k_type, k)] = _deserialize(v_type, v)
          end
        end
      else # model
        SigningTodayAPIClient.const_get(type).build_from_hash(value)
      end
    end

    # Returns the string representation of the object
    # @return [String] String presentation of the object
    def to_s
      to_hash.to_s
    end

    # to_body is an alias to to_hash (backward compatibility)
    # @return [Hash] Returns the object in the form of hash
    def to_body
      to_hash
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        next if value.nil?
        hash[param] = _to_hash(value)
      end
      hash
    end

    # Outputs non-array value in the form of hash
    # For object, use to_hash. Otherwise, just return the value
    # @param [Object] value Any valid value
    # @return [Hash] Returns the value in the form of hash
    def _to_hash(value)
      if value.is_a?(Array)
        value.compact.map { |v| _to_hash(v) }
      elsif value.is_a?(Hash)
        {}.tap do |hash|
          value.each { |k, v| hash[k] = _to_hash(v) }
        end
      elsif value.respond_to? :to_hash
        value.to_hash
      else
        value
      end
    end
  end
end
