  ...
  class IdentityAssociation
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SigningTodayAPIClient::IdentityAssociation` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SigningTodayAPIClient::IdentityAssociation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      provider_id_validator = EnumAttributeValidator.new('String', ["aruba", "aruba-auto", "infocert", "namirial"])
    ...
    def provider_id=(provider_id)
      validator = EnumAttributeValidator.new('String', ["aruba", "aruba-auto", "infocert", "namirial"])
    ...
    def hash
      [provider_id, label, provider_data].hash
    end
