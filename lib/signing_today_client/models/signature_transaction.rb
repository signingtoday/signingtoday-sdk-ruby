  ...
  class SignatureTransaction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SigningTodayAPIClient::SignatureTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SigningTodayAPIClient::SignatureTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'cancelback')
        self.cancelback = attributes[:'cancelback']
      end
    ...
    def valid?
      status_validator = EnumAttributeValidator.new('String', ["waiting", "pending", "performed", "expired", "cancelled"])
      return false unless status_validator.valid?(@status)
      reason_validator = EnumAttributeValidator.new('String', ["CANNOT_DOWNLOAD_DOCUMENT", "INVALID_DOCUMENT", "PROTECTED_DOCUMENT", "declined", "MOTIVAZIONE_ESPLICITA"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["waiting", "pending", "performed", "expired", "cancelled"])
    ...
    def reason=(reason)
      validator = EnumAttributeValidator.new('String', ["CANNOT_DOWNLOAD_DOCUMENT", "INVALID_DOCUMENT", "PROTECTED_DOCUMENT", "declined", "MOTIVAZIONE_ESPLICITA"])
    ...
    def hash
      [id, documents, archived, created_by, status, created, reason, title, not_after, urlback, cancelback, template_name].hash
    end
