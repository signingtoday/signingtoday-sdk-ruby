  ...
  class Bit4idPathgroupOrganizationsApi
    ...
    def get_organization_with_http_info(organization_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: Bit4idPathgroupOrganizationsApi.get_organization ...'
      end
      # verify the required parameter 'organization_id' is set
      if @api_client.config.client_side_validation && organization_id.nil?
        fail ArgumentError, "Missing the required parameter 'organization_id' when calling Bit4idPathgroupOrganizationsApi.get_organization"
      end
      # resource path
      local_var_path = '/organizations/{organization-id}'.sub('{' + 'organization-id' + '}', CGI.escape(organization_id.to_s).gsub('%2F', '/'))
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: Bit4idPathgroupOrganizationsApi#get_organization\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
    ...
    # @param organization_id [String] The **organization-id** represents an organization that is included in the SigninToday application, also know as **slug** and it is used as a path parameter to restrict the asked functionality to the specified organization 
    # @param update_organization [UpdateOrganization] The settings of the organization to edit
    # @param [Hash] opts the optional parameters
    # @return [InlineResponse200]
    def patch_organization(organization_id, update_organization, opts = {})
      data, _status_code, _headers = patch_organization_with_http_info(organization_id, update_organization, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @return [Array<(InlineResponse200, Integer, Hash)>] InlineResponse200 data, response status code and response headers
    def patch_organization_with_http_info(organization_id, update_organization, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: Bit4idPathgroupOrganizationsApi.patch_organization ...'
      end
      # verify the required parameter 'organization_id' is set
      if @api_client.config.client_side_validation && organization_id.nil?
        fail ArgumentError, "Missing the required parameter 'organization_id' when calling Bit4idPathgroupOrganizationsApi.patch_organization"
      end
      ...
      if @api_client.config.client_side_validation && update_organization.nil?
        fail ArgumentError, "Missing the required parameter 'update_organization' when calling Bit4idPathgroupOrganizationsApi.patch_organization"
      end
      # resource path
      local_var_path = '/organizations/{organization-id}'.sub('{' + 'organization-id' + '}', CGI.escape(organization_id.to_s).gsub('%2F', '/'))
      ...
      # http body (model)
      post_body = opts[:body] || @api_client.object_to_http_body(update_organization) 
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: Bit4idPathgroupOrganizationsApi#patch_organization\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
