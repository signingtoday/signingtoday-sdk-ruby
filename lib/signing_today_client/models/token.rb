...
module SigningTodayAPIClient
  ...
  class Token
    ...
    def self.openapi_types
      {
        :'created_by' => :'String',
        :'http_options' => :'CreateTokenHttpOptions',
        :'id' => :'String',
        :'label' => :'String',
        :'user' => :'String',
        :'value' => :'String',
        :'webhooks' => :'CreateTokenWebhooks'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SigningTodayAPIClient::Token` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SigningTodayAPIClient::Token`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [created_by, http_options, id, label, user, value, webhooks].hash
    end
